...
void uncompress_file(const char *input_path, const char *output_path) {
    // Open the input file for reading
    FILE *input_file = fopen(input_path, "rb");
    if (!input_file) {
        fprintf(stderr, "Error: Unable to open input file: %s\n", input_path);
    ...
    if (!snappy_uncompressed_length(input_data, read_size, &uncompressed_size)) {
    ...
    // Compress the data using Snappy
    int status = snappy_uncompress(input_data, input_size, uncompressed_data);
    printf("WORKING SO FAR\n");
    if (status != 0) {
    ...
    printf("File uncompressed successfully: %s\n", output_path);
}

int main(int argc, char *argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <input_file_path> <output_file_path> <uncompressed_file_path>\n", argv[0]);
        return 1;
    }
    const char *input_path = argv[1];
    const char *output_path = argv[2];
    const char *final_path = argv[3];
    compress_file(input_path, output_path);
    uncompress_file(output_path, final_path);
    return 0;
}
